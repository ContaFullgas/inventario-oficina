...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Mantén la imagen actual, a menos que se suba una nueva válida
  $imgName = $item['imagen'];
  ...
  if (!$errors) {
    ...
    // PRG: flash + redirect a pestaña principal
    flash_set('ok', 'Cambios guardados correctamente');
    $dest = "index.php?tab=inv#inv";

    if (!headers_sent()) {
      header("Location: $dest", true, 303); // 303 = POST -> GET
      exit;
    } else {
      // Fallback si ya hubo salida
    ...
    // Si hay errores, dejamos que renderice el formulario con mensajes
    $item['nombre']    = $nombre;
    $item['clase']     = $clase;
...
<body class="container py-4">
  ...
  <form method="post" enctype="multipart/form-data" class="row g-3" action="editar.php?id=<?=intval($id)?>">
    ...
    <div class="col-md-6">
      ...
      <input type="text" name="clase" class="form-control" value="<?=h($item['clase'])?>">
    </div>
    ...
    <div class="col-md-3">
      ...
      <input type="text" name="condicion" class="form-control" value="<?=h($item['condicion'])?>">
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-6">
      ...
      <input type="text" name="ubicacion" class="form-control" value="<?=h($item['ubicacion'])?>">
    </div>
